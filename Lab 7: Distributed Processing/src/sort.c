...
float *merge_tree(int *result_size, float *sub_vec, int n1, int step) {
    ...
    if (rank % step == 0) {
        ...
        if (source >= world_size) {
            ...
            other_vec = malloc(n2 * sizeof(float));
            MPI_Recv(other_vec, n2 * sizeof(float), MPI_INT, source, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        ...
        MPI_Send(sub_vec, sizeof(float) * n1, MPI_INT, dest, 0, MPI_COMM_WORLD);
