...
int main(int argc, char *argv[])
{
  ...
  for(int i = 0; i < repeats; ++i) {
     ...
     */
      ...
      int bytes_read = fread(buffer, 1, chunk_size, src);
      int write_success = fwrite(buffer, 1, bytes_read, dst);
      bzero(buffer, chunk_size);

      while (bytes_read > 0 && write_success > 0) {
          bytes_read = fread(buffer, 1, chunk_size, src);
          write_success = fwrite(buffer, 1, bytes_read, dst);
          bzero(buffer, chunk_size);
      }
