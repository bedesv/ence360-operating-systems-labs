...
void append_buffer(Buffer *buffer, char *data, size_t length) {
    ...
    while (buffer->reserved < buffer->length * sizeof(char) + length) {
        buffer->reserved = 2 * buffer-> reserved;
        buffer->data = realloc(buffer->data, buffer->reserved);
    }
    memcpy(buffer->data + (buffer->length * sizeof(char)), data, length);
...
int main(int argc, char *argv[])
{
  ...
  for(int i = 0; i < repeats; ++i) {
    ...
    FILE *src = fopen("test.jpg", "r");
    if (src == NULL) {
      fprintf(stderr, "error opening test.dat");
      exit(EXIT_FAILURE);
    }

    FILE *dst = fopen("output.jpg", "w");
    if (dst == NULL) {
